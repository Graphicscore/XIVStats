...
#pull image
http POST "$PORTAINER_DOCKER_URL/images/create?fromImage=ghcr.io%2Fgraphicscore%2Fxivstats:latest" "$PORTAINER_TOKEN_HEADER"
...
http POST "$PORTAINER_DOCKER_URL/containers/$CONTAINER_NAME/stop" "$PORTAINER_TOKEN_HEADER"

echo "Renaming old container ..."
#rename old container
http POST "$PORTAINER_DOCKER_URL/containers/$CONTAINER_NAME/rename?name=$CONTAINER_NAME-old" "$PORTAINER_TOKEN_HEADER"
...
http POST "$PORTAINER_DOCKER_URL/containers/$CONTAINER_NAME/start" "$PORTAINER_TOKEN_HEADER"

echo "Deleting old container ..."
http DELETE "$PORTAINER_DOCKER_URL/containers/$CONTAINER_NAME-old?force=true&v=1" "$PORTAINER_TOKEN_HEADER"
